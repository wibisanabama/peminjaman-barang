@extends('layouts.app', ['header' => 'Edit Kategori', 'title' => 'Edit Kategori - InvenTrack'])
@section('content')
<div class="max-w-2xl">
        ...
        <form action="{{ route('kategori.update', $kategori) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="3"
                    class="w-full px-4 py-3 rounded-xl bg-slate-800/50 border border-slate-700/50 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all text-sm resize-none">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
            </div>
